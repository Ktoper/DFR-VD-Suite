...
def export_fea_loads(f_res, r_res):
    """
    Exports static loads to a format ready for SolidWorks/ANSYS.
    """
    import pandas as pd
    
    # Create a simple dataframe to output the solved link forces
    df = pd.DataFrame({
        'Front_Loads_N': f_res,
        'Rear_Loads_N': r_res
    })
    
    df.to_csv("fea_static_loads.csv")
    print("FEA loads exported to fea_static_loads.csv")
